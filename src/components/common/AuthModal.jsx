      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <Field label="Email Address" error={errors.email?.message}>
          <input
            ...
            type="email" placeholder="your@email.com" className={inputCls}
          />
        </Field>
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3.5">
        <div>
          <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2">I am a…</label>
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={() => setValue('role', 'teacher')}
              className={`flex items-center gap-2 p-3 rounded-xl border-2 transition-all ${
                role === 'teacher' ? 'border-blue-600 ring-2 ring-blue-100 bg-blue-50' : 'border-gray-200 hover:border-blue-200 bg-white'
              }`}>
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${role === 'teacher' ? 'bg-blue-600' : 'bg-gray-100'}`}>
                <GraduationCap className={`w-4 h-4 ${role === 'teacher' ? 'text-white' : 'text-gray-400'}`} />
              </div>
              <span className={`text-sm font-bold ${role === 'teacher' ? 'text-blue-600' : 'text-gray-500'}`}>Teacher</span>
            </button>

            <button type="button" onClick={() => setValue('role', 'school')}
              className={`flex items-center gap-2 p-3 rounded-xl border-2 transition-all ${
                role === 'school' ? 'border-amber-400 ring-2 ring-amber-100 bg-amber-50' : 'border-gray-200 hover:border-amber-200 bg-white'
              }`}>
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${role === 'school' ? 'bg-amber-400' : 'bg-gray-100'}`}>
                <School className={`w-4 h-4 ${role === 'school' ? 'text-white' : 'text-gray-400'}`} />
              </div>
              <span className={`text-sm font-bold ${role === 'school' ? 'text-amber-600' : 'text-gray-500'}`}>School</span>
            </button>
          </div>
        ...
        <Field label={role === 'school' ? 'Contact Person Name' : 'Full Name'} error={errors.name?.message}>
          <input {...register('name', { required: 'Name is required' })} placeholder="Enter your full name" className={inputCls} />
        </Field>

        {role === 'school' && (
          <Field label="School Name" error={errors.schoolName?.message}>
            <input {...register('schoolName', { required: 'School name is required' })} placeholder="Enter school name" className={inputCls} />
          </Field>
        )}
        ...
        <Field label="Password" error={errors.password?.message}>
          <div className="relative">
            <input {...register('password', { required: 'Password is required', minLength: { value: 6, message: 'Minimum 6 characters' } })}
              type={showPass ? 'text' : 'password'} placeholder="Minimum 6 characters" className={`${inputCls} pr-11`} />
            <button type="button" onClick={() => setShowPass(p => !p)}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
              {showPass ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            </button>
        ...
        <button type="submit" disabled={loading}
          className={`w-full rounded-xl py-2.5 font-bold text-sm transition-colors disabled:opacity-60 ${
            role === 'school' ? 'bg-amber-400 text-gray-900 hover:bg-amber-300' : 'bg-blue-600 text-white hover:bg-blue-700'
          }`}>
      ...
      <p className="text-center text-xs text-gray-400 mt-4">
        ...
        <button onClick={() => onSwitch('login')} className="text-blue-600 font-bold hover:underline">Sign in</button>
      </p>
    </>
  );
};
    ...
    <>
      ...
      {sent ? (
        <div className="text-center py-4">
          ...
          <button onClick={() => onSwitch('login')} className="mt-5 text-blue-600 text-xs font-bold hover:underline">
            Back to login
          </button>
        </div>
      ) : (
...
// ══════════════════════════════════════════════════════════
const AuthModal = () => {
  // ✅ Reads modal state directly from AuthContext — no separate AuthModalContext needed
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxHeight: '92vh', animation: 'modalIn .25s cubic-bezier(.4,0,.2,1)' }}
